...
gulp.task('styles', function () {
  return gulp
    .src(Config.paths.source.scss + '/ui-cropper.scss')
    .pipe(sass())
    .pipe(
      prefix({
...
gulp.task('scripts', gulp.series('scripts-part-1', 'scripts-part-2'));
...
gulp.task('dist:css:clean', function () {
  return gulp
    .src([Config.paths.compileMinified.root + '/**/*.css'], { read: false })
    .pipe(clean());
});
gulp.task(
  'dist:js',
  gulp.series('scripts', 'dist:js:clean', function () {
    return gulp
...
gulp.task(
  'dist:css',
  gulp.series('dist:css:clean', 'styles', function () {
    return gulp
      .src(Config.paths.compileUnminified.css + '/**/*.css')
      .pipe(cleanCSS())
...
// Build
gulp.task('build', gulp.parallel('dist:js', 'dist:css'));
